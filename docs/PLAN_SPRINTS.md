...
- [x] **Reposicionamiento web → producción (1 ago 2026):** merge `web-y2k` → `main` para desplegar en `soyelaranova.com` (ya no solo preview).
  ...
  - [~] Fase 2.7-2.9 · capas con dirección de Evelyn (commits 4bca67c9→0c93e49b): silueta ciruela con brillos rompiendo el marco blob · sello circular giratorio · marco blob ondulado (refs Spark&Marnee) · tablero de cuadros RECHAZADO por Evelyn ("no somos de cuadros, somos Legally Blonde it girly") y eliminado. Refs nuevas de Evelyn: candy shop lila (marcos festoneados, ticker), Red Hot Studio (pills, sellos), Spark&Marnee (blob masks), pixel phone UI rosa. **JSON pose-01 hero listo** (`docs/prompts/pose-01-hero.json`: fondo verde chroma + candado de cara + candado de marca) — Evelyn genera con su foto de referencia; al recibirla: recortar chroma y montar en la tarjeta del hero. PENDIENTE: pulir secciones inferiores + mobile pass + footer.
  - [ ] Fase 3 · **(decisiones Evelyn 31 jul madrugada)**:
