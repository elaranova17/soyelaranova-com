...
export function ElaraGenerateSlot({
  ...
  return (
    <div
      className={`elara-generate-slot flex flex-col items-center justify-center gap-3 border border-dashed border-[var(--color-purple-amethyst)]/50 bg-gradient-to-br from-[#2D1B69] via-[#1A0F3D] to-[#0E0726] p-6 text-center ${positionClass} ${className}`.trim()}
    ...
    >
      ...
      <code className="rounded-md border border-[var(--color-purple-amethyst)]/30 bg-[#0E0726]/60 px-2 py-1 text-[9px] tracking-wide text-[var(--color-pale-lav)]/55">
        {id}.png
      </code>
    </div>
  )
